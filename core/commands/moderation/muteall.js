...
module.exports = {
  name: "muteall",
  description: "Rend muet tous les membres du salon vocal où vous êtes.",
  category: "moderation",
  usage: "+muteall",
  ...
  async execute(client, message, args) {
        ...
        .reply({
          embeds: [
            client.embedBuilder.error(
              client,
              message.t("commands.muteall.join_voice"),
            ),
          ],
        })
        .catch(() => {});
        ...
        .reply({
          embeds: [
            client.embedBuilder.info(
              client,
              message.t("commands.muteall.no_target"),
            ),
          ],
        })
        .catch(() => {});
    ...
    for (const member of membersInChannel.values()) {
      try {
        await member.voice.setMute(true, `MuteAll par ${message.author.tag}`);
        count++;
      } catch (e) {
        failed++;
    ...
    const finalEmbed = client.embedBuilder
      .success(client, "​")
      .setDescription(null)
      .setAuthor({ name: message.t("commands.muteall.title") })
      .addFields(
        {
          name: message.t("commands.muteall.field_channel"),
          value: `<#${voiceChannel.id}>`,
          inline: true,
        },
        {
          name: message.t("commands.muteall.field_affected"),
          value: fmtNum(count),
          inline: true,
        },
        {
          name: message.t("commands.muteall.field_failed"),
          value: fmtNum(failed),
          inline: true,
        },
        {
          name: message.t("commands.muteall.field_duration"),
          value: `${elapsed} s`,
          inline: true,
        },
      );
