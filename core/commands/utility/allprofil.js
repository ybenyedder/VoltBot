...
function buildEmbed(client, message, items, page, totalPages) {
  ...
  const embed = client.embedBuilder
    .base(client)
    .setAuthor({
      name: message.t("commands.allprofil.author", { count: items.length }),
  ...
  if (totalPages > 1)
    embed.setFooter({
      text: message.t("commands.allprofil.page", {
        page: page + 1,
        total: totalPages,
      }),
    });
...
function buildRow(message, page, totalPages, userId, disabled = false) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`allprofil_prev_${userId}`)
      .setLabel(message.t("commands.allprofil.prev"))
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(disabled || page === 0),
    new ButtonBuilder()
      .setCustomId(`allprofil_page_${userId}`)
      .setLabel(
        message.t("commands.allprofil.page", {
          page: page + 1,
          total: totalPages,
        }),
      )
      ...
      .setDisabled(true),
    new ButtonBuilder()
      .setCustomId(`allprofil_next_${userId}`)
      .setLabel(message.t("commands.allprofil.next"))
      .setStyle(ButtonStyle.Secondary)
...
module.exports = {
  name: "allprofil",
  description: "Affiche les profils de tous les membres",
  category: "utility",
  usage: "allprofil",
  async execute(client, message, args) {
        ...
        .reply({
          embeds: [
            client.embedBuilder.info(
              client,
              message.t("commands.allprofil.no_result"),
            ),
          ],
        })
        .catch(() => {});
    ...
    collector.on("collect", async (i) => {
          ...
          .reply({
            embeds: [
              client.embedBuilder.error(
                client,
                message.t("commands.allprofil.not_author"),
              ),
            ],
            ephemeral: true,
          })
      ...
      if (i.customId.startsWith("allprofil_prev_"))
        page = Math.max(0, page - 1);
      else if (i.customId.startsWith("allprofil_next_"))
